...
async function loadDevices() {
  ...
  deleteError.value = ''
  try {
    const res = await getApiClient().get('devices')
    devices.value = normalizeList(res, 'devices')
  } catch (err) {
    error.value = firstErrorMessage(err, 'Failed to load devices')
    devices.value = []
  } finally {
    loading.value = false
  }
...
async function confirmAndDelete(pkey) {
  ...
  try {
    await getApiClient().delete(`devices/${encodeURIComponent(pkey)}`)
...
<template>
  <div class="list-view">
    <header class="list-header">
      <h1>Devices</h1>
      <p class="toolbar">
        <router-link :to="{ name: 'device-create' }" class="add-btn">Create</router-link>
        <input
          v-model="filterText"
          type="search"
          class="filter-input"
    ...
    <section v-if="loading || error || deleteError || devices.length === 0" class="list-states">
      ...
      <div v-else-if="devices.length === 0" class="empty">
        No devices. (API returned an empty list.)
      </div>
    </section>

    <section v-else class="list-body">
      <p v-if="filterText && filteredDevices.length === 0" class="empty">
      ...
      <table v-else class="table">
        <thead>
          <tr>
            ...
            <th
              class="th-sortable"
              title="Click to sort"
              :class="sortClass('technology')"
              @click="setSort('technology')"
            >
              Technology
            </th>
            <th
        ...
        <tbody>
          <tr v-for="d in sortedDevices" :key="d.pkey">
            ...
            <td>
              <router-link
                :to="{ name: 'device-detail', params: { pkey: d.pkey } }"
            ...
            <td>
              <button
                ...
                :title="deletingPkey === d.pkey ? 'Deleting…' : 'Delete'"
                :aria-label="deletingPkey === d.pkey ? 'Deleting…' : 'Delete'"
                :disabled="deletingPkey === d.pkey"
                @click="askConfirmDelete(d.pkey)"
              >
                <span
                  v-if="deletingPkey === d.pkey"
    ...
    >
      <template #body>
        <p>
          Device template <strong>{{ confirmDeletePkey }}</strong> will be permanently deleted. This
          cannot be undone.
        </p>
      </template>
    </DeleteConfirmModal>
