...
async function confirmAndDeleteIvr(shortuid) {
...
<template>
  <div class="list-view">
    ...
    <section v-if="loading || error || deleteError || ivrs.length === 0" class="list-states">
      <ListLoadingState v-if="loading" message="Loading IVRs from API…" />
          ...
          <tr v-for="ivr in sortedIvrs" :key="ivr.shortuid || ivr.id || (ivr.cluster || '') + '-' + (ivr.pkey || '')" :class="{ 'list-row-inactive': !isRowActive(ivr.active) }">
            ...
            <td>
              <button
                ...
                :aria-label="deletingPkey === ivr.shortuid ? 'Deleting…' : 'Delete'"
