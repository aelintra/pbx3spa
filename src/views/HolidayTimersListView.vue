...
function sortValue(item, key) {
  ...
  if (key === 'stime' || key === 'etime') return Number(item[key]) ?? 0
...
<template>
  <div class="list-view">
    ...
    <section v-if="loading || error || deleteError || holidaytimers.length === 0" class="list-states">
      <p v-if="loading" class="loading">Loading Holiday timers…</p>
    ...
    <section v-else class="list-body">
      <p v-if="filterText && filteredHolidaytimers.length === 0" class="empty">No timers match the filter.</p>
      <table v-else class="table">
        ...
        <tbody>
          <tr v-for="h in sortedHolidaytimers" :key="h.shortuid || h.id || h.pkey">
            ...
            <td>
              <router-link v-if="h.shortuid" :to="{ name: 'holidaytimer-detail', params: { shortuid: h.shortuid } }" class="cell-link cell-link-icon" title="Edit" aria-label="Edit">
