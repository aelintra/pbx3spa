...
async function confirmAndDeleteExtension(pkey) {
  if (confirmDeletePkey.value !== pkey) return
  deleteError.value = ''
  deletingPkey.value = pkey
  try {
    await getApiClient().delete(`extensions/${encodeURIComponent(pkey)}`)
          ...
          <tr v-for="e in sortedExtensions" :key="e.pkey">
            ...
            <td>
              <button
                ...
                :title="deletingPkey === e.pkey ? 'Deleting…' : 'Delete'"
                :aria-label="deletingPkey === e.pkey ? 'Deleting…' : 'Delete'"
                :disabled="deletingPkey === e.pkey"
                @click="askConfirmDelete(e.pkey)"
              >
                <span v-if="deletingPkey === e.pkey" class="action-icon action-icon-spin" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 0 0-9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M3 12a9 9 0 0 0 9 9 9.75 9.75 0 0 0 6.74-2.74L21 16"/><path d="M16 21h5v-5"/></svg></span>
    ...
    <Teleport to="body">
      <div v-if="confirmDeletePkey" class="modal-backdrop" @click.self="cancelConfirmDelete">
        <div class="modal" role="dialog" aria-modal="true" aria-labelledby="modal-delete-title">
          <h2 id="modal-delete-title" class="modal-title">Delete extension?</h2>
          <p class="modal-body">
            Extension <strong>{{ confirmDeletePkey }}</strong> will be permanently deleted. This cannot be undone.
          </p>
          <div class="modal-actions">
            <button type="button" class="modal-btn modal-btn-cancel" @click="cancelConfirmDelete">Cancel</button>
            <button type="button" class="modal-btn modal-btn-delete" :disabled="deletingPkey === confirmDeletePkey" @click="confirmAndDeleteExtension(confirmDeletePkey)">
