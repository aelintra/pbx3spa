...
const { sortKey, sortOrder } = useStickySort('agents', { defaultKey: 'pkey' })
...
function tenantPkeyDisplay(a) {
  if (a.tenant_pkey != null && a.tenant_pkey !== '') return String(a.tenant_pkey)
  const c = a.cluster
  if (c == null || c === '') return '—'
...
function sortValue(a, key) {
  if (key === 'cluster') return tenantPkeyDisplay(a)
  const v = a[key]
...
async function loadAgents() {
  ...
  try {
    ...
    console.log('Agents loaded:', agentsList.length, 'items')
    ...
    try {
      ...
      console.log('Tenants loaded:', tenantsList.length, 'items')
          ...
          <tr v-for="a in sortedAgents" :key="a.shortuid || a.id || (a.cluster || '') + '-' + (a.pkey || '')">
            ...
            <td>
              <button
                ...
                :title="deletingPkey === a.shortuid ? 'Deleting…' : 'Delete'"
                :aria-label="deletingPkey === a.shortuid ? 'Deleting…' : 'Delete'"
