...
const { sortKey, sortOrder } = useStickySort('help-messages', { defaultKey: 'pkey' })
...
async function loadMessages() {
  ...
  try {
    ...
    messages.value = normalizeList(res, 'helpcore')
  } catch (err) {
    error.value = firstErrorMessage(err, 'Failed to load help messages')
    messages.value = []
  } finally {
    loading.value = false
  }
...
async function confirmAndDelete(pkey) {
  ...
  try {
    await getApiClient().delete(`helpcore/${encodeURIComponent(pkey)}`)
...
<template>
  <div class="list-view">
    <header class="list-header">
      <h1>Help messages</h1>
      <p class="toolbar">
        <router-link :to="{ name: 'help-message-create' }" class="add-btn">Create</router-link>
        <input
          v-model="filterText"
          type="search"
          class="filter-input"
    ...
    <section v-if="loading || error || deleteError || messages.length === 0" class="list-states">
      ...
      <div v-else-if="messages.length === 0" class="empty">
        No help messages. (API returned an empty list.)
      </div>
    </section>

    <section v-else class="list-body">
      <p v-if="filterText && filteredMessages.length === 0" class="empty">
        No help messages match the filter.
      </p>
      <table v-else class="table">
        ...
        <tbody>
          <tr v-for="m in sortedMessages" :key="m.pkey">
            ...
            <td>
              <router-link
                :to="{ name: 'help-message-detail', params: { pkey: m.pkey } }"
            ...
            <td>
              <button
                ...
                :title="deletingPkey === m.pkey ? 'Deleting…' : 'Delete'"
                :aria-label="deletingPkey === m.pkey ? 'Deleting…' : 'Delete'"
                :disabled="deletingPkey === m.pkey"
                @click="askConfirmDelete(m.pkey)"
              >
                <span
                  v-if="deletingPkey === m.pkey"
    ...
    >
      <template #body>
        <p>
          Help message <strong>{{ confirmDeletePkey }}</strong> will be permanently deleted. This
          cannot be undone.
        </p>
      </template>
    </DeleteConfirmModal>
