...
<template>
  <div class="list-view">
    ...
    <section v-else class="list-body">
      <p v-if="filterText && filteredMessages.length === 0" class="empty">No help messages match the filter.</p>
        ...
        <tbody>
          <tr v-for="m in sortedMessages" :key="m.pkey">
            ...
            <td>
              ...
              >
                <span v-if="deletingPkey === m.pkey" class="action-icon action-icon-spin" aria-hidden="true">
    ...
    >
      <template #body>
        <p>Help message <strong>{{ confirmDeletePkey }}</strong> will be permanently deleted. This cannot be undone.</p>
      </template>
    </DeleteConfirmModal>
  </div>
</template>
