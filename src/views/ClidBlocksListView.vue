...
const { filterText } = useStickyFilter('clidblocks')
...
const { sortKey, sortOrder } = useStickySort('clidblocks', { defaultKey: 'pkey' })
...
async function loadRows() {
  ...
  error.value = ''
  try {
    const [listRes, tRes] = await Promise.all([
      getApiClient().get('clidblocks'),
      loadTenantOptions()
    ])
    rows.value = normalizeList(listRes, 'clidblocks') || normalizeList(listRes) || []
    tenants.value = tRes
  } catch (err) {
    error.value = firstErrorMessage(err, 'Failed to load blocked caller IDs')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="list-view">
    <header class="list-header">
      <h1>Blocked caller IDs</h1>
      <p class="list-legend">
        Inbound calls from listed caller IDs are rejected for that tenant. No Commit required — active
        rows apply on the next call.
      </p>
      <p class="toolbar">
        <router-link :to="{ name: 'clidblock-create' }" class="add-btn">Create</router-link>
        <input
          v-model="filterText"
          type="search"
          autocomplete="off"
    ...
    <section v-if="loading || error || deleteError || rows.length === 0" class="list-states">
      <ListLoadingState v-if="loading" message="Loading blocked caller IDs…" />
      <p v-else-if="error" class="error">{{ error }}</p>
      <p v-if="deleteError" class="error">{{ deleteError }}</p>
      <div v-else-if="rows.length === 0" class="empty">No blocked caller IDs.</div>
    </section>

    <section v-else class="list-body">
      <p v-if="filterText && filteredRows.length === 0" class="empty">No rows match the filter.</p>
      <table v-else class="table">
        ...
        <tbody>
          <tr v-for="r in sortedRows" :key="r.shortuid || r.id">
            <td class="cell-immutable" title="Immutable">{{ r.pkey ?? '—' }}</td>
            ...
            <td>
              <router-link
                v-if="r.shortuid"
                :to="{ name: 'clidblock-detail', params: { shortuid: r.shortuid } }"
    ...
    >
      <template #body>
        <p>
          ...
          <strong>{{
            rows.find((r) => r.shortuid === confirmDeleteShortuid)?.pkey ?? confirmDeleteShortuid
          }}</strong>
