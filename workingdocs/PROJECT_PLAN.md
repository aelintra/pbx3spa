...
**Done:** Steps 1–17 plus Steps 18+ (Tenants, Extensions, Trunks, Queues, Agents, Routes, IVRs, Inbound routes, Custom Apps — full CRUD where applicable; list/detail/create refactored per PANEL_PATTERN.md: list blocks, detail Identity/Settings/Advanced, edit-from-list, delete confirmation modal, toasts). Create-button wording standardized: list toolbar = “Create”, create form submit = “Create” / “Creating…”. Default after login is **Home** (dashboard): PBX status via `GET syscommands/pbxrunstate` + Refresh, and actions Commit / Start / Stop / Reboot with confirmations. Auth: login, sessionStorage persist, route guard, whoami in layout.
