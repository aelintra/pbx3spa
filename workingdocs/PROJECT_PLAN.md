...
**Last session:** Help messages (tt_help_core) panel: API (HelpCore model, HelpCoreController, helpcore routes + schema), SPA (list/create/detail, router, nav, sticky filter, validateHelpCorePkey). Docs: SESSION_HANDOFF, PROJECT_PLAN, EDIT_PANEL_FIELD_PARITY_AUDIT, HOLISTIC_ASSESSMENT, UX_IMPROVEMENTS_IVR updated; per-field hint To-do added with UX approach (always-visible short hints vs on-demand tooltips). Branch: **spanel** (pbx3spa, pbx3api). See SESSION_HANDOFF.md for next-agent pointers. Earlier: Create-panel standardization (§3) and UX (no selectable dash in dropdowns; Tenant Advanced toggles; pills for 2–3 options); Inbound route create/detail hardening and schema alignment. API: move_request_to_model uses request->all() for JSON; InboundRouteController pkey from request, Asterisk extension validation, single-0 reject; models aligned to pbx3/full_schema.sql. Frontend: Inbound route detail Open/Close = destination dropdowns; MOH NO→OFF. full_schema.sql = schema yardstick; boolean standardisation doc + migration added (not run). Branches: newpanels (pbx3api), wizards (pbx3spa). See SESSION_HANDOFF.md for full latest-session summary. (Earlier: UI standardization: list panels use a single **Create** button (no “Add {object}”); create-form submit buttons use **Create** / **Creating…** (no “Create {object}”). All create views updated (Extension, Trunk, Route, Queue, Agent, IVR, Tenant, Inbound route). Branch **extensions-buildout** was merged into **main** (fast-forward); **extensions-buildout** deleted locally. User was to run `git push origin main` and `git push origin --delete extensions-buildout` from their machine (credentials required). Work is on **main**.

**Done:** Steps 1–17 plus Steps 18+ (Tenants, Extensions, Trunks, Queues, Agents, Routes, IVRs, Inbound routes, Custom Apps, Help messages (tt_help_core) — full CRUD where applicable; list/detail/create refactored per PANEL_PATTERN.md: list blocks, detail Identity/Settings/Advanced, edit-from-list, delete confirmation modal, toasts). Create-button wording standardized: list toolbar = “Create”, create form submit = “Create” / “Creating…”. Default after login is **Home** (dashboard): PBX status via `GET syscommands/pbxrunstate` + Refresh, and actions Commit / Start / Stop / Reboot with confirmations. Auth: login, sessionStorage persist, route guard, whoami in layout.
