...
**Done – Field mutability (API):** API exposes GET /schemas (read_only, updateable, defaults per resource). Frontend uses useSchema composable (cache, no Pinia); all eight detail views derive read_only from schema; all eight create views preset from schema.defaults. See **FIELD_MUTABILITY_API_PLAN.md** and **pbx3api/docs/SCHEMAS_ENDPOINT.md**.
