...
- **Row backgrounds:** `list-row-inactive` (grey) uses only the extension row’s **Active** field from the list API. **`list-row-status-unknown`** (amber) applies only after `statusDisplay(e) === 'Unknown'`, i.e. when **`extensions/live`** has finished — until then IP/Status show `…` and the row is **not** tinted unknown. Do **not** add a blanket “loading” row class for all active rows: it reads like the whole table is inactive/grey. Global `main.css` includes higher-specificity rules for `td.cell-immutable` inside tinted rows so scoped list CSS cannot leave immutable columns stuck on `#f8fafc`.
