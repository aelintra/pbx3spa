...
- Loads `extensions` + `tenants` first, then `extensions/live` so the table is not blocked by AMI (see `ExtensionsListView.vue`). `liveData` holds the live response object. **`LiveDataFetchNotice`** sits on the **same row** as **`ListViewMeta`** (`.list-meta-toolbar` in `main.css`): row counts on the left, compact live-status pill on the right while `extensions/live` runs. **Trunks** uses the same pattern during `trunks/live`.
- **Extensions** and **Trunks** lists: **Active?** / **Active** use **`ListActiveChip`**, **Latency** uses **`ListLiveLatencyChip`** + **`liveLatencyChip.js`** (same thresholds: **Unknown** / **Live…** / **`N ms`** with green **&lt;100**, yellow **100–200**, orange **201–300**, red **&gt;300**). Shared pill styles: **`main.css`** (`.list-view .list-chip`, …). Other entity lists (IVRs, queues, inbound/outbound routes, tenants, CoS, conferences, custom apps, day timers) use **`ListActiveChip`** only (no row grey tint).
- **IP** and **Latency** columns: `ipDisplay(e)` and `statusDisplay(e)` use `String(e.pkey)` to look up `liveData.value[key]`. Keys in the API response are strings (e.g. `"1000"`, `"1110"`); `e.pkey` from the list is also string in JSON, but normalising with `String(e.pkey)` avoids any number/string mismatch.
