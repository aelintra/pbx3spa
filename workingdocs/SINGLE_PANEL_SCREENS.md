...
| 7 | **Certificates** | sarkcert       | ✅ API + panel | **Done.** TLS server certs: Let's Encrypt individual cert (status + renew) and purchased cert (install/remove). Route `/certificates`, CertificatesView.vue. API: certificates/active, certificates/letsencrypt, certificates/letsencrypt/renew, certificates/custom (GET/POST/DELETE). See CERTIFICATES_ADOPTION_PLAN.md. |
