...
| 7 | **Certificates** | sarkcert       | ❌ No API      | TLS server certs: Let's Encrypt (status + renew) and purchased/custom cert (install/remove). pbx3 owns LE; panel for LE status + custom cert only. **Plan:** CERTIFICATES_ADOPTION_PLAN.md. |
