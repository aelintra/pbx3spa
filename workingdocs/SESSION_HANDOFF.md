...
- **PANEL_PATTERN.md** §8 — reference implementation status; §3 for create-form rules; §2.2 list blocks; §4.1 detail blocks; **App shell** — **SessionContextChips** / context row; **Edit** — **detail-panel-head**, **DetailActiveStatusBar**, **detail-active-inactive-hint**; **PanelBackLink** + top `←` parent navigation on create/detail (and dashboard-parent settings views).
- **SPA_SHELL_ROADMAP.md** — Stage 1 + Stage 2 shell work (context chips, detail active UX); remaining ideas (collapsible sidebar, ⌘K).
