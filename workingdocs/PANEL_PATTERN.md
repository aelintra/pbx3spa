...
- **Top bar context (instance / tenant):** **`SessionContextChips`** in **AppLayout** shows **Instance** (prefer **`sysglobals.fqdn`**) and optional **Tenant** when tenant detail is open. Layout: dedicated **`.topbar-center`** with **viewport-centered** positioning (transform uses **`var(--pbx-shell-sidebar-width)`** — keep in sync with **`.sidebar`** width). **`.topbar-left`** holds **`PBX3 Admin`**; do not paint opaque backgrounds on the left/right wings over the chips (stacking context — see **SESSION_HANDOFF** § Shell / topbar). Implementation: **`auth`** store, **`useSessionContext`**, **`sessionContext.js`**; **`SessionContextChips.vue`**.
- **Sidebar branding:** No logo in the nav column for now; **`.sidebar-top-spacer`** preserves top spacing. Replace with a real logo asset when ready.
