...
This folder is **for the AI (Cursor agent)** to record progress and build a memory of the pbx3-frontend system. When you start a new chat, read the files here to get up to speed.
...
| **PROJECT_PLAN.md** | Discrete job steps (Vue stack): scaffold → API client → auth → login → layout → Tenants/Extensions list → …; test, sign off, commit per step. |
